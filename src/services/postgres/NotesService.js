...
class NotesService {
  ...
  async addNote({
    ...
    const query = {
      text: 'INSERT INTO notesapp.notes VALUES($1, $2, $3, $4, $5, $6, $7) RETURNING id',
  ...
  async editNoteById(id, { title, body, tags }) {
    ...
    const query = {
      text: 'UPDATE notesapp.notes SET title = $1, body = $2, tags = $3, updated_at = $4 WHERE id = $5 RETURNING id, owner',
  ...
  async deleteNoteById(id) {
    const query = {
      text: 'DELETE FROM notesapp.notes WHERE id = $1 RETURNING id, owner',
      values: [id],
    };
    const result = await this._pool.query(query);
    if (!result.rows.length) {
